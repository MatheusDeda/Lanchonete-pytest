...
def test_post_e_get_cliente(client):
    ...
    Este teste cobre o fluxo completo de criação e leitura:
        1. POST /lanchonete/clientes  → cria o cliente e retorna seus dados
        2. GET /lanchonete/clientes/{cpf} → busca o cliente cadastrado
    ...
    """
    response = client.post("/lanchonete/clientes", json={"cpf": "11122233344", "nome": "Cliente X"})
    assert response.status_code == 200
    assert response.json()["cpf"] == "11122233344"

    response2 = client.get("/lanchonete/clientes/11122233344")
...
def test_get_cliente_inexistente(client):
    ...
    """
    response = client.get("/lanchonete/clientes/000")
